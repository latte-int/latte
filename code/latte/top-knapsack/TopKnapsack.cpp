...
/**
 * @parm v is inserted at the end of the list if it is not already there.
 */
 ...
 * @parm i, index of the current mu value that will be computed. On first call, this index value must be the gcd=1 case.
 */
 ...
 *
 * @parm k starting with B[0], computes up to B[k], where the B's are the Bernoulli of the 1st kind:
 */
 ...
 * @parm t: if true, uses the every-subsets method, else uses the complete method.
 */
...
/**
 * @parm k will compute the coefficients of t^N, t^{N-1}, ..., t^{N-k}
 */
...
/**
 * @parm k will compute the coefficient of t^{N-k}
 */
...
/**
 * @parm k: gets the coeff of t^{N-k}. Assumes coeff_NminusK or coeff_topK was already called
 */
 ...
 *
 * @parm latticeBasis: output parameter. The cols contain the the basis such that \sum latticeBasis[:,i]*\Z \in f\Z
 * @parm fnDivAlpha: list of numbers
 * @parm f: number
 */
 ...
 * @parm latticeBasis: if x = latticeBasis*y for any y, then \sum alapha_i x_i \in f\Z
 * @parm invLatticeBasisD = determinant(latticeBasis)
 * @parm invLatticeBasis matrix such that invLatticeBasisScaled * latticeBasis = determinant(latticeBasis) * I
 * @parm invLatticeBasisScaled = invLatticeBasis but each column scaled to have gcd 1
 */
...
/**
 * @parm invLatticeBasis: the columns contains rays for a cone.
 ...
 *
 * @parm fSeries: output parameter. Is the series expansion of the above, but with an extra x^{N+1} scaling term.
 ...
 * @parm s: the vertex of the cone
 * @parm u: list of unimodular cones
 * @parm B: colums are a basis for the lattice of all solutions for \sum \alpha_i x_i in f\Z
 * @parm invB: matrix such that B*invB/invBd = I
 * @parm invBd: det(B)
 * @parm fnDivAlpha list of alpha's that f does Not divide
 * @parm fDivAlpha list of alpha's  that f does divide
 */
 ...
 * @parm fDivAlpha: list of numbers
 * @parm a: output polynomial. will be cleared before we start. Is equal to 1/(\prod_{i: f does not divide \alpha_i} (1-exp(alaph_i*x))) * alpha_i*x*-1
 */
...
*
* @parm bottomCoeffPeriodicPart. output parameter. scaling term.
* @parm a. output parameter. the polynomial expansion with missing scaling term.
* @parm numPoles is the number of zeros in expa
**/
 ...
 *
 * @parm expansion, an output parameter, assumed to be initialized to zero, else its value will be lost.
 */
