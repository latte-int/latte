...
void checkListVector(listVector* basis, int numOfVars) {
	...
	while (basis) {
		...
		if ((IsZero(counter) == 1) && (RHS < 0)) {
			...
			out << 0 << endl;
			exit(0);
		} else if ((IsZero(counter) == 1) && (RHS > 0))
			removeListVector(basis);
	}
	/*  printf("\n"); */
	return;
}
...
/* ----------------------------------------------------------------- */
listVector* preprocessProblem(listVector *equations, listVector *inequalities,
	...
	if ((flag == 1) || (flag == 0)) {
		...
		for (i = 0; i < *numOfVars; i++) {
			...
			for (k = 0; k < (*numOfVars); k++) {
				Proj[i][0] = Proj[i][0] + tmpProjU[i][k + 1]
						* particularSolution[k];
			}
			...
			for (j = 1; j < newNumOfVars; j++) {
				ProjU[i][j] = 0;
				for (k = 0; k < (*numOfVars); k++) {
					Proj[i][j] = Proj[i][j] + tmpProjU[i][k + 1]
							* (tmp2->first)[k];
				}
				tmp2 = tmp2->rest;
			}
			for (int m = 0; m < newNumOfVars; m++)
				ProjU[i][m] = Proj[i][m];
		} //cerr << ProjU << endl;
