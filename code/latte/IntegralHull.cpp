...
vector SolveIP(listCone* cones,listVector* matrix,listVector* matrix2, vector cost, int numOfVars, int SINGLE_CONE)
{
  //int SINGLE_CONE = 1;
	...
	vector OptVertex;
  	int  flag = 0;
  	listCone * tmpcone;
  	listVector  *tmpmatrix, *tmpmatrix2;
  	tmpcone = cones;
  	list< vector > solutions;
  	vector		Temp_Vector;
	...
	while(1)
	{
		...
		if (Coefficient_Sum == 0)
		{
			...
			while (ConeInfo_List_Highest_Terms != NULL)
			{
				...
				DigLevel = *ConeInfo_List_Highest_Terms->ConeInfo_Pointer->Get_Current_Highest_Term();
			...
			//	cout << endl << "Done Digging " << Digging_Count << endl;
			...
			//{
				...
				while(!solutions2.empty())
				{
					...
					{ 
					  RHS = 0; //cout <<"after while: " << possible << endl;
      						for(int i = 0; i < numOfVars; i++)
							RHS += (tmpmatrix2 -> first)[i + 1] * possible[i];
      						if((tmpmatrix2 -> first[0] + RHS) != 0)
						{
							// flag = 1 means infeasible
							flag = 1;
          						break;
	...
	*/
	if(Digging_Count != 0){
	  cout << endl << "Digging Depth: " << Digging_Count << endl;
	  cout << "Digging Level: " << OptVertex * cost - DigLevel << endl;
        } else cout << endl << "No digging." << endl;
	return OptVertex;
...
listVector* GetVertices(listCone* cones, listVector* matrix, listVector* matrix2, listVector* hyperplane, int numOfVars, int flag)
{
	...
	if(flag == 0)
	{
    		if(hyperplane == 0)
		{
      			...
      			for(int j = 0; j < numOfVars; j++)
			{
				...
				vertex = SolveIP(cones, matrix, matrix2,  cost, numOfVars, 0); // cout << vertex << endl;
    		...
    		else
		{
      			...
      			while(tmpVertices)
			{
				vertex = SolveIP(cones, matrix, matrix2, tmpVertices -> first, numOfVars, 0);  
				endVertex -> rest = createListVector(vertex);
				endVertex = endVertex -> rest;   
  	...
  	else if(flag == 1)
	{
    		...
    		for(int i = 0; i < numOfVars; i++)  
			cin >> cost[i];
    		vertex = SolveIP(cones, matrix, matrix2, cost, numOfVars, 0);
    		endVertex -> rest = createListVector(vertex);
    		endVertex = endVertex -> rest; 
  	}
...
listVector* IntegralHull(listCone* cones,listVector* matrix, listVector* matrix2,int numOfVars)
{
	...
	if (IntegralHull_Flag == 1)
	{
		...
		vertices = GetVertices(cones, matrix, matrix2, 0, numOfVars, 0);
	
		for(int i = 0; i < numOfVars; i++)
		{
		
    			vertices = Push_Vector(vertices,GetVertices(cones, matrix, matrix2, 0, numOfVars, 0), numOfVars);
    		}
   		...
   		while(counter != 0)
		{
   	 		...
   	 		newVertices = GetVertices(cones, matrix, matrix2, hyperplanes, numOfVars, 0);
	...
	else if (IntegralHull_Flag == 0)
	{
		...
		while (!Cost_File.eof())
		{
			...
			SolveIP (cones, matrix, matrix2,  Cost_Vector, numOfVars, 0);
