...
Polyhedron * building_polyhedron_easy(const ReadProblemType readProblemType, BarvinokParameters * parms);
Polyhedron * building_polyhedron_vrepToVrep();
...
int main(int argc, char *argv[]) {
	cout
			<< "Hello, this file is designed to help developers use LattE as\n"
	...
	building_polyhedron_easy(hrepToCones, params);
	building_polyhedron_easy(hrepToVertices, params);
	building_polyhedron_easy(vrepToCones, params);
	building_polyhedron_easy(vrepToVertices, params);
 ...
 *
 * This function is very inefficient is the number line of code.
 * One should place the dualize function
 * calls inside "if statements" that branch on if the polyhedron is dualized
 * or homogenized.
 *
 */
Polyhedron * building_polyhedron_easy(const ReadProblemType readProblemType, BarvinokParameters * params) {
	...
	return Poly;
}//building_polyhedron_easy()




#if 0
//This is commented out because there is no easy way to get the tangent cones
//  starting from a v/h rep matrix, and there is no easy way to get the v-rep
//  polyhedron from a h-rep matrix.
//However, this function works perfectly well.

/**
 * We will generate a homogenized Polyhedron from a v-rep (this is easy).
 */
Polyhedron * building_polyhedron_vrepToVrep() {
	dd_MatrixPtr M = getVrepMatrix();
	Polyhedron *poly;

	//the memory in M will be freed.
	poly = PolyhedronFromVrepMatrix(M, true /*always needs to be true */);

	//now, poly contains the rays of the polytope.

	//feel free to play with the next print block to see what things look like.
#if 1
	cout << "Number of vars" << poly->numOfVars << endl;
	cout << "homogenized: " << poly->homogenized << endl; //true: cones represent the homogenization of the polyhedron.
	//false:cones represent the supporting cones of all vertices of the polyhedron.
	cout << "dualized: " << poly->dualized << endl;
	printListCone(poly->cones, poly->numOfVars); //note how everything is saved in 1 cone.
#endif

	return poly;
}//example_building_polyhedron()
#endif
...
void finding_integral_polynomial_coneDecomp()
{
	RationalNTL ans;
	BarvinokParameters *params = new BarvinokParameters;
	Polyhedron * poly = building_polyhedron_easy(hrepToCones, params);
	...
	ans = polytopeValuation.findIntegral(polynomial, PolytopeValuation::LawrenceIntegration);
...
void finding_integral_linearForm_triangulation()
{
	RationalNTL ans;
	BarvinokParameters * params = new BarvinokParameters;
	Polyhedron * poly = building_polyhedron_easy(hrepToVertices, params);
	PolytopeValuation polytopeValuation(poly, *params);
	linFormSum lForms = getLinearForms();

	ans = polytopeValuation.findIntegral(lForms, PolytopeValuation::TriangulationIntegration);
...
void finding_volume_coneDecomp()
{
	...
	BarvinokParameters *params = new BarvinokParameters;
	cout << "hell" << endl;
	Polyhedron * poly = building_polyhedron_easy(vrepToCones, params);
	cout << "duee" << endl;
	PolytopeValuation polytopeValuation(poly, *params);
...
void finding_volume_triangulation()
{
	RationalNTL ans;
	BarvinokParameters *params = new BarvinokParameters;
	Polyhedron * poly = building_polyhedron_easy(vrepToVertices, params);
	PolytopeValuation polytopeValuation(poly, *params);
