...
testfrac := [[2, 1, 4], [2, 8, 7], [4, 3, 2], [2, 8, 2]]; testdim2:=[[1, 1], [1, 0], [0, 1]];testdim4:=[[6,9,5,1],[3,5,4,0],[7,4,9,1],[1,3,7,2],[8,9,1,7]];
...
plot([conetestfrac0,conetestfrac1,conetestfrac2,conetestfrac3],t=0..1,color=[green,blue,red,black], axis[1]=[gridlines=[20, thickness=1, subticks=false]],axis[2]=[gridlines=[7, thickness=1, subticks=false]],thickness=2);
eval(subs(t=0.4, conetestfrac3));
...
conetestdim2; 
SS:=simplify(1+ourfrac(t)^2-(2*(-ourfrac(-t)+1/2))*ourfrac(t)+(1/2)*ourfrac(-t)^2-(3/2)*ourfrac(-t)-(3/2)*ourfrac(t)-(-ourfrac(t)+1/2)*ourfrac(t)+(3/2-2*ourfrac(t)-ourfrac(-t))*t+(1/2)*t^2);
IntSS:=factor(1+(3/2)*t+(1/2)*t^2);
seq(eval(subs({t=i/10,ourfrac=bonfrac},SS)),i=0..400);
