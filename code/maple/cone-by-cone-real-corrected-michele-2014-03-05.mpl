...
#print(i,S[i],W,eval(subs({TODD=Todd,EXP=exp},dilated_approxi_cone(n,S[i],W,order,xi))));
    od:
...
cone_by_cone_real:=proc(Simplex,ell,M,order) local reg,d,xx,AA,CCt,CCeps,CCn;
d:=nops(Simplex)-1;CCn:=0;
#order:=M+nops(Simplex)-codim; 
reg:=random_vector(5000,d); 
xx:=[seq(t*(ell[i]+epsilon*reg[i]),i=1..d)];
