...
#  We compute the knapsack for A:
#  the number of  integral solutions for A_1x_1+..+A_{N+1}x_{N+1}=t.
# We output this as a step polynomial function  of t: 
# sum_i E_i(t) t^i .
...
# 
# Main final commands;
# 1. complete_knapsack:=proc(A,t). Computes knapsack for A1x1+..A_{N+1}x_{N+1}=t, t a variable or an integer. The output is either a quasi polynomial function of t; expressed in function of t^k and the function frac(t)=t-floor(t);
# 
# 2. coeff_Nminusk_knapsack:=proc(A,t,k)  computes the approximation given by the N-k-ehrhart coeff; The output is a periodic function of t;
# THIS IS THE MAIN OBJECTIVE OF THIS PROGRAM;
# 3. To test some values, we have numeric_knapsack:=proc(A,t);
# 
# Here are easy examples; Run the program
# before; 
...
#fi;
#end:
fract:=proc(a,n) local out,p,q,newa,t; if  a=0 then out:=0; 
elif type(simplify(a),integer)=true  then out:=0;
else 
p:=numer(a); q:=denom(a); 
newa:=modp(p,q);
...
coeff_Nminusk_knapsack:=proc(A,t,k) local out,N,F,i,c,p,f;
out:=0;
N:=nops(A)-1;
...
#print("i=",i,"c=", c);
if f=1 then 
out:=out+c*part_one_of_knapsack(A,N-k)*t^(N-k);
else 
print("k=",k);
out:=out+c*part_f_of_knapsack(A,T,f,N-k)*t^(N-k);fi ;
od;
eval(subs(T=t,simplify(out)));
end:
